...
public class BoardDAO {
	...
	final String sql_selectAll="SELECT * FROM BOARD ORDER BY BID DESC LIMIT 0,?"; //더보기 기능
		// LIMIT 은 MySQL
		// Oracle은 ROWNUM 을 사용함 (*SQL문 실행순서 유의!)
	...
	final String sql_update_F="UPDATE BOARD SET FAVCNT=FAVCNT+1 WHERE BID=?"; // 좋아요
	
	public ArrayList<BoardSet> selectAll(BoardVO bvo){ // '인자' >> 유지보수 용이 
		...
		try {
			...
			while(rs.next()) {
				...
				boardVO.setRcnt(rs.getInt("RCNT")); // 댓글수 // rList.size();로 하면 컬럼을 줄이고 서버 비용을 줄일 수 있음!
	...
	public boolean updateF(BoardVO bvo) {
		conn=JDBCUtil.connect();
		try {
			pstmt=conn.prepareStatement(sql_update_F);
			pstmt.setInt(1, bvo.getBid());
			pstmt.executeUpdate();
		}catch (SQLException e) {
			e.printStackTrace();
